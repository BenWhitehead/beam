...
public final class FirestoreV1FnBatchGetDocumentsTest extends
    BaseFirestoreV1ReadFnTest<BatchGetDocumentsRequest, BatchGetDocumentsResponse> {
  ...
  public void endToEnd() throws Exception {
    final BatchGetDocumentsRequest request = BatchGetDocumentsRequest.newBuilder()
    ...
    when(rpc.batchGetDocumentsCallable()).thenReturn(callable);

    when(ff.getFirestoreRpc(any())).thenReturn(rpc);
    when(ff.getRpcQos(any())).thenReturn(FirestoreStatefulComponentFactory.INSTANCE.getRpcQos(RPC_QOS_OPTIONS));

    ArgumentCaptor<BatchGetDocumentsResponse> responsesCaptor = ArgumentCaptor.forClass(
  ...
  public void resumeFromLastReadValue() throws Exception {
    ...
    final BatchGetDocumentsRequest request1 = BatchGetDocumentsRequest.newBuilder()
        .setDatabase(String.format("projects/%s/databases/(default)", projectId))
    ...
    BatchGetDocumentsRequest request2 = BatchGetDocumentsRequest.newBuilder()
        .setDatabase(String.format("projects/%s/databases/(default)", projectId))
        .addDocuments(response3.getMissing())
        .addDocuments(response4.getFound().getName())
        .build();
    BatchGetDocumentsRequest request3 = BatchGetDocumentsRequest.newBuilder()
        .setDatabase(String.format("projects/%s/databases/(default)", projectId))
        .addDocuments(response4.getFound().getName())
        .build();

    when(responseStream1.iterator())
        .thenReturn(new AbstractIterator<BatchGetDocumentsResponse>() {
    ...
    when(responseStream3.iterator())
        .thenReturn(ImmutableList.of(response4).iterator());
    ...
    BatchGetDocumentsFn fn = new BatchGetDocumentsFn(clock, ff, RPC_QOS_OPTIONS);
