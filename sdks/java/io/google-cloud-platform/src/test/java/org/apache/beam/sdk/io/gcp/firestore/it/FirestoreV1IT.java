...
public final class FirestoreV1IT extends BaseFirestoreIT {
  private static final Logger LOGGER = LoggerFactory.getLogger(FirestoreV1IT.class);
  ...
  public void batchWrite_partialFailureOutputsToDeadLetterQueue()
    ...
    Write validWrite = Write.newBuilder()
        .setUpdate(
            Document.newBuilder()
    ...
    helper.getBaseDocument().collection(collectionId).document(docId2)
        .create(ImmutableMap.of("foo", "baz")).get(10, TimeUnit.SECONDS);

    // this will fail because we're setting a updateTime precondition to before it was created
    Write conditionalUpdate = Write.newBuilder()
        .setUpdate(
            Document.newBuilder()
    ...
    RpcQosOptions options = BaseFirestoreIT.rpcQosOptions.toBuilder()
        .withBatchMaxCount(2)
        .build();
    PCollection<WriteFailure> writeFailurePCollection = testPipeline.apply(Create.of(writes))
        .apply(
    ...
    ApiFuture<QuerySnapshot> actualDocsQuery = helper.getBaseDocument().collection(collectionId)
        .orderBy("__name__").get();
    QuerySnapshot querySnapshot = actualDocsQuery.get(10, TimeUnit.SECONDS);
    List<QueryDocumentSnapshot> documents = querySnapshot.getDocuments();
    List<KV<String, String>> actualDocumentIds = documents.stream()
        .map(doc -> KV.of(doc.getId(), doc.getString("foo")))
        .collect(Collectors.toList());

    List<KV<String, String>> expected = newArrayList(
        KV.of(docId, docId),
        KV.of(docId2, "baz")
    );
  ...
  @Override
  protected PTransform<PCollection<String>, PCollection<PartitionQueryRequest>> getPartitionQueryPTransform(
      String testMethodName) {
    return new PartitionQuery(helper.getDatabase(), helper.getDocumentRoot());
  }
  ...
  private static final class ListCollectionIds extends BasePTransform<String, ListCollectionIdsRequest> {
    ...
    public PCollection<ListCollectionIdsRequest> expand(PCollection<String> input) {
      return input.apply(ParDo.of(new DoFn<String, ListCollectionIdsRequest>() {
        @ProcessElement
  ...
  private static final class ListDocuments extends BasePTransform<String, ListDocumentsRequest> {
    ...
    public PCollection<ListDocumentsRequest> expand(PCollection<String> input) {
      return input.apply(ParDo.of(new DoFn<String, ListDocumentsRequest>() {
        @ProcessElement
        public void processElement(ProcessContext c) {
          c.output(
  ...
  private static final class BatchGetDocuments extends BasePTransform<List<String>, BatchGetDocumentsRequest> {
    ...
    public BatchGetDocuments(String database, String baseDocumentPath,
        String collectionId) {
    ...
    public PCollection<BatchGetDocumentsRequest> expand(PCollection<List<String>> input) {
      return input.apply(ParDo.of(new DoFn<List<String>, BatchGetDocumentsRequest>() {
        @ProcessElement
        public void processElement(ProcessContext c) {
  ...
  private static final class RunQuery extends BasePTransform<String, RunQueryRequest> {
    ...
    public PCollection<RunQueryRequest> expand(PCollection<String> input) {
      return input.apply(ParDo.of(new DoFn<String, RunQueryRequest>() {
        @ProcessElement
        public void processElement(ProcessContext c) {
          c.output(
              RunQueryRequest.newBuilder()
                  ...
                  .setStructuredQuery(
                      StructuredQuery.newBuilder()
                          .addFrom(CollectionSelector.newBuilder().setCollectionId(c.element()))
                          .setWhere(
                              Filter.newBuilder()
                                  .setFieldFilter(
  ...
  private static final class PartitionQuery extends BasePTransform<String, PartitionQueryRequest> {
    ...
    public PartitionQuery(String database, String baseDocumentPath) {
      super(database, baseDocumentPath, "");
    }

    @Override
    public PCollection<PartitionQueryRequest> expand(PCollection<String> input) {
      return input.apply(ParDo.of(new DoFn<String, PartitionQueryRequest>() {
        @ProcessElement
        public void processElement(ProcessContext c) {
          c.output(PartitionQueryRequest.newBuilder()
              .setParent(baseDocumentPath)
              .setPartitionCount(3)
              .setStructuredQuery(
                  StructuredQuery.newBuilder()
  ...
  private static final class WritePTransform extends BasePTransform<List<String>, Write> {
    ...
    public PCollection<Write> expand(PCollection<List<String>> input) {
      return input.apply(ParDo.of(new DoFn<List<String>, Write>() {
        @ProcessElement
        public void processElement(ProcessContext c) {
  ...
  private static abstract class BasePTransform<In, Out> extends PTransform<PCollection<In>, PCollection<Out>> {

    protected final String database;
    protected final String baseDocumentPath;
    protected final String collectionId;

    private BasePTransform(
        String database, String baseDocumentPath, String collectionId) {
