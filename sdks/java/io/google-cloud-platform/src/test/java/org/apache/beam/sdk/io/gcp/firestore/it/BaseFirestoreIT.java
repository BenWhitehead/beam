...
abstract class BaseFirestoreIT {
  ...
  protected static final int NUM_ITEMS_TO_GENERATE = 768; // more than one read page and one write page
  ...
  public final void listCollections() throws Exception {
    ...
    List<String> collectionIds = IntStream.rangeClosed(1, 20)
        .mapToObj(i -> helper.colId())
    ...
    PCollection<String> actualCollectionIds = testPipeline
        .apply(Create.of(""))
        .apply(getListCollectionIdsPTransform(testName.getMethodName()))
        .apply(FirestoreIO.v1().read().listCollectionIds().withRpcQosOptions(rpcQosOptions).build());
  ...
  public final void listDocuments() throws Exception {
    ...
    PCollection<String> listDocumentPaths = testPipeline
        .apply(Create.of("a"))
  ...
  public final void runQuery() throws Exception {
    String collectionId = "a";
    DocumentGenerator documentGenerator = helper.documentGenerator(NUM_ITEMS_TO_GENERATE, collectionId, /* addBazDoc = */ true);
    documentGenerator.generateDocuments().get(10, TimeUnit.SECONDS);
  ...
  public final void partitionQuery() throws Exception {
    ...
    PCollection<String> listDocumentPaths = testPipeline
        ...
        .apply(getPartitionQueryPTransform(testName.getMethodName()))
  ...
  public final void batchGet() throws Exception {
    ...
    PCollection<String> listDocumentPaths = testPipeline
        ...
        .apply(FirestoreIO.v1().read().batchGetDocuments().withRpcQosOptions(rpcQosOptions).build())
  ...
  protected abstract PTransform<PCollection<String>, PCollection<ListCollectionIdsRequest>> getListCollectionIdsPTransform(String testMethodName);

  protected abstract PTransform<PCollection<String>, PCollection<ListDocumentsRequest>> getListDocumentsPTransform(
      String testMethodName);

  protected abstract PTransform<PCollection<List<String>>, PCollection<BatchGetDocumentsRequest>> getBatchGetDocumentsPTransform(
      String testMethodName, String collectionId);

  protected abstract PTransform<PCollection<String>, PCollection<RunQueryRequest>> getRunQueryPTransform(
      String testMethodName);

  protected abstract PTransform<PCollection<String>, PCollection<PartitionQueryRequest>> getPartitionQueryPTransform(String testMethodName);

  protected final void runWriteTest(
      PTransform<PCollection<List<String>>, PCollection<Write>> createWrite,
      String collectionId) {
    List<String> documentIds = IntStream.rangeClosed(1, 1_000)
        .mapToObj(i -> helper.docId())
        .collect(Collectors.toList());

    // Create.of unwraps the list of document ids, so wrap it in another list
    testPipeline.apply(Create.of(Collections.singletonList(documentIds)))
        .apply(createWrite)
        .apply(FirestoreIO.v1().write().batchWrite().withRpcQosOptions(rpcQosOptions).build());

    testPipeline.run(options);

    List<String> actualDocumentIds = helper.listDocumentsViaQuery(String.format("%s/%s", helper.getBaseDocumentPath(), collectionId))
