...
public final class FirestoreV1FnRunQueryTest extends
    BaseFirestoreV1ReadFnTest<RunQueryRequest, RunQueryResponse> {
  ...
  public void endToEnd() throws Exception {
    TestData testData = TestData.fieldEqualsBar()
    ...
    when(responseStream1.iterator())
        .thenReturn(responses.iterator());

    when(callable.call(testData.request)).thenReturn(responseStream1);

    when(rpc.runQueryCallable()).thenReturn(callable);
    ...
    ArgumentCaptor<RunQueryResponse> responsesCaptor = ArgumentCaptor.forClass(
  ...
  public void resumeFromLastReadValue() throws Exception {
    TestData testData = TestData.fieldEqualsBar()
        .setProjectId(projectId)
        .setOrderFunction(f -> Collections.singletonList(
            Order.newBuilder()
    ...
    RunQueryRequest request2 = RunQueryRequest.newBuilder()
    ...
    List<RunQueryResponse> responses = ImmutableList.of(testData.response1, testData.response2, testData.response3);
    when(responseStream1.iterator())
        .thenReturn(new AbstractIterator<RunQueryResponse>() {
    ...
    RunQueryFn fn = new RunQueryFn(clock, ff, RPC_QOS_OPTIONS);
    ...
    RunQueryRequest request2 = RunQueryRequest.newBuilder()
        ...
        .setStructuredQuery(
            testData.request.getStructuredQuery().toBuilder()
                ...
                .addOrderBy(
                    Order.newBuilder().setField(FieldReference.newBuilder().setFieldPath("__name__")).setDirection(Direction.ASCENDING)
                )
        )
        .build();
  ...
  @Override
  protected RunQueryFn getFn(JodaClock clock,
  ...
  private static final class TestData {
    ...
    public TestData(
        String projectId,
        Function<FieldReference, List<Order>> orderFunction
    ) {
      String fieldPath = "foo";
      FieldReference foo = FieldReference.newBuilder()
          .setFieldPath(fieldPath).build();
      ...
      request = RunQueryRequest.newBuilder()
          .setParent(String.format("projects/%s/databases/(default)/document", projectId))
    ...
    private static RunQueryResponse newResponse(String field, int docNumber) {
      ...
      return RunQueryResponse.newBuilder()
          .setDocument(
              Document.newBuilder()
                  .setName(docId)
                  .putAllFields(ImmutableMap.of(field, Value.newBuilder().setStringValue("bar").build()))
                  .build()
          )
          .build();
    }
    ...
    private static final class Builder {
      ...
      public Builder setOrderFunction(
          Function<FieldReference, List<Order>> orderFunction) {
